...
export default function WorkspacesIndex({ workspaces }: { workspaces: WorkspaceWithBoards[] }) {
  ...
  return (
    <AppLayout breadcrumbs={breadcrumbs}>
      ...
      <div className="mx-auto w-full max-w-2xl space-y-8 p-6">
        <Heading title="Workspaces" description="Manage your workspaces and create new ones." />
            ...
            >
              <div className="flex size-8 items-center justify-center rounded-md bg-primary/10 text-primary">
                {ws.is_personal ? (
                  ...
                  <span className="font-bold text-sm">{ws.name.charAt(0).toUpperCase()}</span>
                )}
              </div>
              <div className="flex-1">
                <div className="font-medium">{ws.name}</div>
                <div className="text-muted-foreground text-xs">
                  {ws.boards.length} {ws.boards.length === 1 ? "board" : "boards"}
                </div>
              </div>
              {ws.is_personal && (
