...
export default function WorkspaceMembers({ workspace, members, invites, role }: Props) {
  ...
  const handleCreateInvite = () => {
    router.post(WorkspaceInviteController.store.url(workspace), {}, { preserveScroll: true });
  };
  ...
  return (
    <AppLayout breadcrumbs={breadcrumbs}>
      ...
      <div className="mx-auto w-full max-w-2xl space-y-8 p-6">
        <Heading title="Members" description="Manage workspace members and invitations." />
            ...
            return (
              <div key={member.id} className="flex items-center gap-3 rounded-lg border p-3">
                ...
                {canManage && !isOwner ? (
                  ...
                  <span className="rounded bg-muted px-2 py-1 text-muted-foreground text-xs capitalize">
                    {member.role}
                  </span>
                )}
          ...
          <div className="space-y-4">
            ...
            {invites.length > 0 && (
              <div className="space-y-2">
                {invites.map((invite) => (
                  <div key={invite.id} className="flex items-center gap-2 rounded-lg border p-3">
                    ...
                    <Button
                      ...
                      onClick={() => copy(getInviteUrl(invite))}
                    >
            ...
            {copiedText && <p className="text-green-600 text-xs">Link copied to clipboard!</p>}
          </div>
        )}
      </div>
        ...
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={handleRemoveMember}>
              {removingMember?.id === auth.user?.id ? "Leave" : "Remove"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
