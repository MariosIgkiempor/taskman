...
export function NotificationBell() {
  ...
  const handleMarkAsRead = useCallback((id: string) => {
    ...
    void requestJson("patch", NotificationController.markAsRead.url(id)).then(() => {
      router.reload({ only: ["unreadNotificationsCount"] });
    });
  }, []);
  ...
  const handleMarkAllAsRead = useCallback(() => {
    setNotifications([]);
    void requestJson("post", NotificationController.markAllAsRead.url()).then(() => {
      router.reload({ only: ["unreadNotificationsCount"] });
    });
  }, []);
        ...
        <div className="max-h-64 overflow-y-auto">
          ...
          {notifications.map((notification) => (
            <button
              key={notification.id}
              type="button"
              className="flex w-full items-start gap-2 border-border/30 border-b px-3 py-2 text-left transition-colors last:border-0 hover:bg-accent"
              onClick={() => handleMarkAsRead(notification.id)}
            >
              <Bell className="mt-0.5 size-3.5 shrink-0 text-primary" />
              <div className="min-w-0 flex-1">
                <p className="truncate font-medium text-xs">{notification.data.task_title}</p>
                <p className="text-muted-foreground text-xs">
                  {formatReminderLabel(notification.data.minutes_before)}
                </p>
              </div>
