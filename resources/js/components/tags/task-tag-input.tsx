...
export function TaskTagInput({
  ...
  const selectedIds = useMemo(() => new Set(taskTags.map((t) => t.id)), [taskTags]);
        ...
        <div className="max-h-40 overflow-y-auto">
          ...
          {showCreate && (
            <button
              type="button"
              className={`flex w-full items-center gap-2 rounded-sm px-2 py-1.5 text-xs ${
                highlightIndex === suggestions.length ? "bg-accent text-accent-foreground" : ""
              }`}
          ...
          {suggestions.length === 0 && !showCreate && (
            <div className="px-2 py-1.5 text-muted-foreground text-xs">No tags available</div>
          )}
        </div>
      </PopoverContent>
    </Popover>
