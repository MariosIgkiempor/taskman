...
export function recurrenceLabel(series: RecurrenceSeries): string {
  const { frequency, interval, days_of_week, month_day, month_week_ordinal, month_week_day } =
    series;
...
export const RecurrencePicker = forwardRef<RecurrencePickerHandle, RecurrencePickerProps>(
  function RecurrencePicker({ initialRule, scheduledDate, onSave, onRemove }, ref) {
    ...
    const [monthWeekDay, setMonthWeekDay] = useState(initialRule?.month_week_day ?? defaultWeekday);
    ...
    const buildRule = (): RecurrenceRule => ({
      ...
      month_week_day: frequency === "monthly" && monthlyMode === "ordinal" ? monthWeekDay : null,
    ...
    const initialSnapshot = useMemo(() => {
      ...
      const initEndMode = initialRule?.end_date
        ? "date"
        : initialRule?.end_count
          ? "count"
          : "never";
      ...
      defaultOrdinal,
      defaultWeekday,
      initialRule?.days_of_week,
      initialRule?.end_count,
      initialRule?.end_date,
      initialRule?.frequency,
      initialRule?.interval,
      initialRule?.month_day,
      initialRule?.month_week_day,
      initialRule?.month_week_ordinal,
    ]);
    ...
    const isDirty = (): boolean => {
      const current = JSON.stringify({
        ...
        initDays: daysOfWeek,
        initMonthlyMode: monthlyMode,
        initMonthDay: monthDay,
        initOrdinal: monthWeekOrdinal,
        initWeekDay: monthWeekDay,
        initEndMode: endMode,
        initEndDate: endDate,
    ...
    return (
      <div className="flex flex-col gap-3 px-3 py-2">
              ...
              {DAY_LABELS.map((label, i) => {
                ...
                return (
                  <button
                    ...
                    className={`flex size-7 items-center justify-center rounded-full font-medium text-[10px] transition-colors ${
                      isSelected
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <label className="flex cursor-pointer items-center gap-1.5">
              ...
              <Select
                ...
                onValueChange={(v) => setMonthWeekOrdinal(parseInt(v, 10))}
              ...
              <Select
                ...
                onValueChange={(v) => setMonthWeekDay(parseInt(v, 10))}
                disabled={monthlyMode !== "ordinal"}
              >
                <SelectTrigger size="sm" className="h-7 w-20 text-xs">
                  <SelectValue />
        ...
        <div className="flex items-center gap-1.5">
          ...
          <Button size="sm" className="ml-auto h-7 px-3 text-xs" onClick={handleSave}>
            Done
          </Button>
        </div>
      </div>
