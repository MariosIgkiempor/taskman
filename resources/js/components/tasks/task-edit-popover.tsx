...
function TaskEditForm({
  ...
  const handleOpenDirections = () => {
    ...
    const arrivalTime = new Date(new Date(task.scheduled_at).getTime() - 15 * 60 * 1000);
  ...
  const handleClearSchedule = () => {
    ...
    setScheduleTime("");
    router.patch(
      TaskController.unschedule.url(task.id),
      {},
      {
        preserveScroll: true,
      },
  ...
  const handleToggleTag = useCallback(
    (tagId: number) => {
      ...
      void requestJson<Tag[]>("patch", TaskTagController.sync.url(currentTask.id), {
        tag_ids: newIds,
      })
        .then(() => {
          router.reload();
  ...
  const handleCreateTag = useCallback(
    (name: string, color: string) => {
      ...
      void (async () => {
        try {
          const response = await requestJson<Tag>("post", TagController.store.url(workspace), {
            name,
            color,
          });
          onTagCreated(response);
          const previousTags = taskTagsRef.current;
          const nextTags = [...previousTags, response].sort((a, b) => a.name.localeCompare(b.name));

          taskTagsRef.current = nextTags;
          setTaskTags(nextTags);

          try {
            await requestJson<Tag[]>("patch", TaskTagController.sync.url(currentTask.id), {
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="flex items-center gap-1 px-2 pt-2">
        ...
        >
          {task.is_completed ? <Check className="size-4" /> : <Circle className="size-4" />}
        </Button>
        <Input
          ref={titleRef}
          value={title}
          onChange={(e) => handleTitleChange(e.target.value)}
          className="flex-1 border-0 px-0 font-semibold text-sm shadow-none focus-visible:ring-0"
      ...
      <div className="px-3 pb-1">
        <textarea
          ...
          rows={2}
          className="w-full resize-none bg-transparent text-muted-foreground text-xs outline-none placeholder:text-muted-foreground/50"
        />
      </div>
      ...
      {/* Actions bar */}
      <div className="flex items-center gap-1 border-border/50 border-t px-2 py-1.5">
        <Button
          variant="ghost"
          size="sm"
      ...
      {showReminderPicker && task.scheduled_at && (
        <div className="border-border/50 border-t">
          <ReminderPicker
            taskId={task.id}
            initialReminders={task.reminders.map((r) => r.minutes_before)}
          />
        </div>
      )}
    </div>
